...
public class LinkedConfigMap extends LinkedHashMap<String, String> {
  ...
  /**
   * Constructs an empty {@code LinkedConfigMap}, using default validation rules. Keys and values
   * must be non-blank.
   */
   ...
   * @throws IllegalArgumentException if any key or value in the map is invalid (null, empty, or
   *                                  blank).
   */
   ...
   * @throws NullPointerException     if {@code map} is {@code null}.
   * @throws IllegalArgumentException if any key or value in the input map is invalid (null, empty,
   *                                  or blank).
   */
  @Override
  ...
  /**
   * Replaces the entry for the specified key only if it is currently mapped to some value, after
  ...
  /**
   * Attempts to compute a mapping for the specified key using the given remapping function. If the
   * result is non-null, it is validated before being stored.
   *
   * @param key               key with which the specified value is to be associated.
   ...
   * compute its value. If the result is non-null, it is validated before being stored.
   *
  ...
  /**
   * Attempts to merge the given value with the existing value for the specified key using the
   * remapping function. If the result is non-null, it is validated before being stored.
   *
